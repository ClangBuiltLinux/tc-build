...
def invoke_ninja(args, dirs, stage):
    ...
    build_folder = dirs.build_folder.joinpath("stage%d" % stage).as_posix()
...
def generate_pgo_profiles(dirs):
    """
    ...
    # Run kernel/build.sh
    subprocess.run([dirs.root_folder.joinpath("kernel", "build.sh")],
                   check=True,
