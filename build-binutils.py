...
def invoke_configure(build_folder, install_folder, root, target):
    configure = [root.joinpath(utils.current_binutils(), "configure").as_posix(),
...
def invoke_make(build_folder, install_folder, target):
    if target == "host":
        subprocess.run(['make', '-s', 'configure-host', 'V=0'], check=True, cwd=build_folder.as_posix())
    subprocess.run(['make', '-s', '-j' + str(multiprocessing.cpu_count()), 'V=0'], check=True, cwd=build_folder.as_posix())
    subprocess.run(['make', '-s', 'prefix=' + install_folder.as_posix(), 'install', 'V=0'], check=True, cwd=build_folder.as_posix())
...
def main():
    ...
    for_all_targets(root.joinpath("build", "binutils"), install_folder, root, create_targets(args.targets))
